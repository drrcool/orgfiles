...
* 2023-01-30:
** Jesus this week has been hell.
** we lost dylan this weekend and I'm struggling a lot with that.  I think this is as close to broken hearted as I have ever been.
** Projects:
*** Qoedash:
**** Previously pushed changes to the DRD views to make addressable fraction the default view
**** TODO There are some problems with the ETL pipeline. Investigate why addressable population isn't working as expected.
**** Pushed changes to fix the build issues we've been facing
**** Pushed changes to the filter drawer for qoedash that converts the filter selector to Hawkins.
    This both solves some problems that the inputs when searching the filters wasn't rendering.  Now they do render properly. It also fixed a QoL issue where selecting an item cleared search as that made finding, for example, all Comcast devices, hard as you had to re-search after each selection.
    Also added the tooltips back for the context help on the filters. They had been invisible due to being rendered on the body while the drawer was in a portal.
